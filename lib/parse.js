...
import { getCooked, printPath } from '@bablr/agast-helpers';
        ...
        case 'guard': {
          ...
          if (['NodeMatcher', 'TerminalMatcher'].includes(matcher.type)) {
            if (!s.path.reference) {
              ...
              const isSyntactic = m.grammar.syntactics.has(type);
              const flags = isSyntactic ? { syntactic: true } : {};

              if (isNode && !isCover) {
                yield buildCall('reference', 'root');
                yield buildCall('startNode', flags, type, rootMatcher.attributes);
              }
            }
            ...
            if ((isNode || isCover) && s.result.type !== 'Reference' && effects.success === 'eat') {
              yield buildCall('reference', strPath);
            }

            if (isNode && !isCover) {
              start = yield buildCall('startNode', flags, type, attributes);
            }
