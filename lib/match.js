...
import { getCooked } from '@bablr/ast-helpers';
import { getProduction } from '@bablr/class-grammar-helpers';
import { WeakStackFrame } from '@bablr/weak-stack';
import { buildTerminalProps } from '@bablr/agast-vm-helpers';
...
export class Match extends WeakStackFrame {
  ...
  exec(state, effects, matcher, props) {
    ...
    const co = new Coroutine(
      ...
      getProduction(grammar, type).apply(grammar, [props_, state, ctx]),
    );
  ...
  capture() {
    ...
    if (!range[0] || (range[0].type === 'OpenNodeTag' && state.result === range[0])) return null;
