...
export class Match extends WeakStackFrame {
  ...
  static from(context, language, state, matcher, productionEnhancer) {
    return Match.create(context, language, state, matcher, productionEnhancer, null, null);
  }
  ...
  exec(state, effects, matcher, props) {
    const { ctx, productionEnhancer, languageRelativePath, declaredPath } = this;
    const { grammars } = ctx;
    const { type } = matcher;
    ...
    const match = this.push(
      ...
      effects,
      co,
      isNode ? undefined : languageRelativePath,
