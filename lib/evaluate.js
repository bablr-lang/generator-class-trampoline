...
function* parseStrategy(ctx, rootMatcher, rootValue, s) {
  ...
  while (co) {
        ...
        case 'holdForMatch': {
          ...
          if (!isRegexPattern && matcher.nodeMatcher?.type !== sym.gap && !isString(matcher)) {
            ...
            if (sourceNodeMatcher.type === Symbol.for('ArrayNodeMatcher')) {
              ...
              if (!s.node.has(name)) {
                yield buildCall(
                  'advance',
                  buildEmbeddedTag(
                    buildReferenceTag(name, true, { hasGap: gapsAllowed && flags.hasGap }),
                  ),
                );
            ...
            if (!isHold && isNode && effects.success === 'eat' && !s.reference) {
              let innerParsedResolvedPath =
                isNode || isCover
                  ? parsedResolvedPath ||
                    m.declaredPath ||
                    (m.pathDepth ? s.path.reference : parseRef('.'))
                  : null;

              const { name, isArray, flags } = innerParsedResolvedPath;

              const flags_ = flags.hasGap ? getFlagsWithGap(referenceFlags) : referenceFlags;

              if (isArray && !s.node.has(name)) {
                yield buildCall('advance', buildEmbeddedTag(buildReferenceTag(name, true, flags_)));
                yield buildCall('advance', buildEmbeddedTag(buildArrayInitializerTag()));
              }
              ...
              const mergedReference =
                reduce(
                  ...
                  [
                    ...concat(
                      [buildReferenceTag(name, isArray, flags_)],
                      map(
                        (m) => {
              ...
              start = buildOpenNodeTag(
                {
                  ...flags,
                  hasGap:
                    s.reference.value.name === '@'
                      ? false
                      : s.reference.value.flags.hasGap || (flags.token ? false : gapsAllowed),
                },
                absoluteLanguage,
                Symbol.for(type),
            ...
            if (selfClosing) {
              ...
              yield buildCall(
                'advance',
                buildEmbeddedTag(buildCloseNodeTag(Symbol.for(type), nodeMatcher.open.language)),
              );
    ...
    finishedMatch = finishedMatch || m;

    {
      // resume suspended execution
      const { type, isNode, grammar, matcher, captured } = finishedMatch;
      const allowEmpty = !!grammar.emptyables?.has(type);
      const wasRejected = m.s !== finishedMatch.s;
      ...
      if (!wasRejected && isNode && !captured) {
        yield buildCall(
          'advance',
          buildEmbeddedTag(buildCloseNodeTag(Symbol.for(type), matcher.language)),
        );
      }
