      ...
      switch (verb) {
        ...
        case 'holdForMatch': {
          ...
          if (
            ...
            yield buildCall('advance', buildEmbeddedTag(buildReference(name, true)));
            start = yield buildCall('advance', buildEmbeddedTag(buildArray()));
            ...
            if (path && effects.success === 'eat') {
              ...
              if (!hasOwn(s.node.properties, name)) {
                if (isArray) {
                  yield buildCall('advance', buildEmbeddedTag(buildReference(name, true)));
                  yield buildCall('advance', buildEmbeddedTag(buildArray()));
                } else {
                  yield buildCall('advance', buildEmbeddedTag(buildReference(name)));
                ...
                if (isNode || isCover) {
                  ...
                  if (
                    ...
                    if (!s.node.flags.trivia && !s.node.flags.escape) {
                      ...
                      yield buildCall(
                        'advance',
                        buildEmbeddedTag(isArray ? buildArray() : buildNull()),
                      );
                    }
                  }
                }
              ...
              start = yield buildCall(
                ...
                buildEmbeddedTag(
                  buildNodeOpenTag(
                    flags,
                    absoluteLanguage,
                    ctx.symbols.get(language).get(type),
            ...
            if (selfClosing) {
              ...
              yield buildCall(
                'advance',
                buildEmbeddedTag(buildNodeCloseTag(ctx.symbols.get(language)?.get(type), matcher.language)),
              );
      ...
      if (!m.zombie) {
        if (!throwing) {
          if (isNode && !isCover && !captured) {
            yield buildCall(
              'advance',
              buildEmbeddedTag(buildNodeCloseTag(ctx.symbols.get(m.language).get(type), startTag.value.language)),
            );
          }
        }
