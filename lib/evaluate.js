...
import { getCooked, getRoot } from '@bablr/agast-helpers/tree';
...
function* parseStrategy(ctx, rootMatcher, rootValue, s) {
  ...
  yield buildCall(
    'advance',
    buildEmbeddedTag(buildFragmentOpenTag({ hasGap: true }, m.language.canonicalURL)),
  );
          ...
          if (
            ...
            if (shouldInterpolate) {
              ...
              start = yield buildCall(
                ...
                buildEmbeddedTag(
                  buildNodeOpenTag(
                    { ...flags, hasGap: flags.escape ? false : flags.hasGap || !flags.token },
                    absoluteLanguage,
                    Symbol.for(type),
