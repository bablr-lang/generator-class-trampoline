...
import { ReferenceTag, GapTag, NullTag, LiteralTag } from '@bablr/agast-helpers/symbols';
...
function buildTag(tag) {
  switch (tag.type) {
    case LiteralTag:
      return buildLiteralTag(tag.value);
    case GapTag:
      return buildGapTag();
    default:
      throw new Error();
...
function* parseStrategy(ctx, rootMatcher, rootValue, s) {
  let m = Match.from(
    ctx,
    ctx.languages.get(
      getCooked(rootMatcher.properties.open.properties.language.properties.content),
    ),
    s,
  );
    ...
    instrLoop: for (;;) {
      ...
      switch (verb) {
        ...
        case 'holdForMatch': {
          ...
          let sourceMatcher = btree.getAt(0, sourceInstr.properties.arguments.properties.values);
          let start;
          ...
          if (
                ...
                if (isNode || isCover) {
                  ...
                  if (
                    ...
                    if (!s.node.flags.trivia && !s.node.flags.escape) {
                      yield buildCall(
                        'advance',
                        buildEmbeddedTag(buildReferenceTag(name, isArray)),
                      );
            ...
            if (
              ...
              const strPath =
                isNode || isCover
                  ? !path || path.type === NullTag
                    ? m.declaredPath || printPath(s.path.reference?.value)
                    : reifyExpression(path)
                  : null;
              const { name, isArray } = parsePath(strPath);

              if (isArray && !hasOwn(s.node.properties, name)) {
                yield buildCall('advance', buildEmbeddedTag(buildReferenceTag(name, true)));
                yield buildCall('advance', buildEmbeddedTag(buildArrayTag()));
              }

              yield buildCall('advance', buildEmbeddedTag(buildReferenceTag(name, isArray)));
            }
            ...
            if (selfClosing) {
              for (const tag of ctx.agast.allTagsFor(intrinsicResult)) {
                if (tag.type === LiteralTag) {
                  yield buildCall('advance', buildEmbeddedTag(buildLiteralTag(tag.value)));
                } else {
                  throw new Error();
                }
              }
              ...
              const sourceProps = btree.getAt(
                2,
                sourceInstr.properties.arguments.properties.values,
              );
            ...
            const pattern = btree.getAt(0, sourceInstr.properties.arguments.properties.values);
      ...
      if (!m.zombie) {
        if (!throwing) {
          if (isNode && !isCover && !captured) {
            yield buildCall(
              'advance',
              buildEmbeddedTag(buildNodeCloseTag(Symbol.for(type), startTag.value.language)),
            );
          }
        }
