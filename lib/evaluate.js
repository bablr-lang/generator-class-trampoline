...
//   //     Math.min(
//   //       s.path.depth + (didShift || s.resultPath.child.type === ReferenceTag ? 0 : 1),
...
//   //     !didShift &&
//   //     !lowNode.has(name) &&
//   //     !(s.resultPath.type === ReferenceTag && s.resultPath.value.name === name)
//   //   ) {
        ...
        case 'holdForMatch': {
          ...
          if (!isRegexPattern && matcher.nodeMatcher?.type !== sym.gap && !isString(matcher)) {
            ...
            const shouldInterpolate =
              atGap && (isNode || isCover) && parsedResolvedPath?.hasGap && !options?.suppressGap;
...

    {
      ...
      matchReturnValue =
        throwing || finishedMatch.rangePreviousIndex == null
          ? null
          : new FragmentFacade(
              finishedMatch.rangePrevious.path,
